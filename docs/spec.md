...
7. 登入後的左側主選單需支援桌面版展開／收合並保存瀏覽器偏好；收合時保留語系、品牌與導覽圖示。手機版預設顯示不超過 68px 的單列列首，僅保留品牌、語系與導覽開關；完整導覽、健康狀態及帳號操作由浮動面板呈現，切頁、點擊背景或按 Escape 後自動收合。
8. 介面需在 1440、1024、768、390px 寬度載入真實資料時維持整頁無水平溢出；1180px 以下主選單改為緊湊頂部導覽，Dashboard 在 820px 以下使用單欄自然流。Team／帳號管理的寬表格只允許於表格容器內橫向捲動，客戶操作列與系統設定欄位需依可用寬度換行。
...
- RWD 驗收需以真實 seed data 登入後檢查 Dashboard、客戶工作台、團隊分析、帳號管理與系統設定，禁止由 `body` 或 `documentElement` 產生水平捲動。
