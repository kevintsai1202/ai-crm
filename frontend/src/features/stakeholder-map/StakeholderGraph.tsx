...
export function StakeholderGraph({ roles, relations }: StakeholderGraphProps) {
  return (
    <div data-testid="sm-confirmed-graph" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div>
        <div style={{ fontSize: 13, fontWeight: 700, color: "#475569", marginBottom: 6 }}>已確認角色</div>
        {roles.length === 0 && <p style={{ fontSize: 13, color: "#94a3b8", margin: 0 }}>尚無已確認角色。</p>}
          ...
          {roles.map((role) => (
            <div key={role.id} data-testid={`sm-role-${role.contactId}`} style={{
              ...
              <div style={{ fontSize: 11, color: "#166534", marginTop: 2 }}>
                {role.roleType}・影響力 {role.influence}・立場 {role.stance}
              </div>
            </div>
          ))}
        </div>
      ...
      <div>
        <div style={{ fontSize: 13, fontWeight: 700, color: "#475569", marginBottom: 6 }}>已確認關係</div>
        {relations.length === 0 && <p style={{ fontSize: 13, color: "#94a3b8", margin: 0 }}>尚無已確認關係。</p>}
