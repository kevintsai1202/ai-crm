...
/** 將建議摘要為可讀文字。 */
function suggestionSummary(suggestion: StakeholderSuggestionDto): string {
  if (suggestion.kind === "ROLE" && suggestion.role) {
    const r = suggestion.role;
    return `${r.contactName}${r.contactTitle ? `（${r.contactTitle}）` : ""}：${r.roleType}・影響力 ${r.influence}・立場 ${r.stance}`;
  }
...
export function StakeholderMapTab() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setError(e instanceof Error ? e.message : "載入決策鏈失敗");
      } finally {
        setLoading(false);
      }
    })();
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto", padding: "24px 16px" }}>
      <h1 style={{ fontSize: 22, fontWeight: 800, color: "#122232", marginBottom: 6 }}>決策鏈</h1>
      <p style={{ color: "#64748b", fontSize: 14, marginBottom: 20 }}>
        已確認的角色與關係為事實（實線）；AI 建議為待確認（虛線），需人工確認後才成為事實。
      </p>

      {error && <div data-testid="sm-error" style={{ color: "#b91c1c", fontSize: 13, marginBottom: 12 }}>⚠️ {error}</div>}
      {loading && <div data-testid="sm-loading" style={{ color: "#475569" }}>載入中…</div>}
      ...
      {map && (
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div data-testid="sm-suggestions">
            <div style={{ fontSize: 13, fontWeight: 700, color: "#475569", marginBottom: 6 }}>AI 待確認建議</div>
            {map.suggestions.length === 0 && <p style={{ fontSize: 13, color: "#94a3b8", margin: 0 }}>目前沒有待確認建議。</p>}
                ...
                <div key={suggestion.suggestionId} data-testid={`sm-suggestion-${suggestion.suggestionId}`}
                  ...
                  <div style={{ minWidth: 0 }}>
                    <span style={{ fontSize: 11, color: "#6d28d9", background: "#ede9fe", padding: "1px 6px",
                      borderRadius: 4, marginRight: 8 }}>待確認</span>
                    <span style={{ fontSize: 14, color: "#122232" }}>{suggestionSummary(suggestion)}</span>
                  </div>
                  <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                    <button type="button" className="btn-primary" data-testid={`sm-confirm-${suggestion.suggestionId}`}
                      ...
                      style={{ padding: "4px 12px", fontSize: 13 }}>確認</button>
                    <button type="button" className="btn-danger" data-testid={`sm-reject-${suggestion.suggestionId}`}
                      ...
                      style={{ padding: "4px 12px", fontSize: 13 }}>拒絕</button>
                  </div>
                </div>
              ))}
            </div>
