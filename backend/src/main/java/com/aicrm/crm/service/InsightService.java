...
public class InsightService {
    ...
    public SseEmitter streamModelScore(Dtos.AiScoreRequest request) {
        ...
        sb.append("""
                ...
                - **內容品質與深度**（40 分）：分析是否有層次、建議行動是否具體可執行、風險脈絡說明是否完整、報告整體是否達到業務主管可直接使用的水準；僅命中客戶姓名但說明空洞者不得超過 20 分
                - **Token 效率／資訊密度**（15 分）：以較少 completion token 完成具體且完整的說明者得高分（資訊密度高）；輸出 token 多但內容空洞、重複或僅列風險分數而無實質分析者扣分；token 少但內容豐富勝過 token 多但空泛
