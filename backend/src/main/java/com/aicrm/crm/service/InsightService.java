...
public class InsightService {
    ...
    public SseEmitter streamModelScore(Dtos.AiScoreRequest request) {
        ...
        sb.append("""
                ...
                - **內容品質與深度**（40 分）：分析是否有層次、建議行動是否具體可執行、風險脈絡說明是否完整、報告整體是否達到業務主管可直接使用的水準；**若回答僅將原始資料重新排列（資料堆砌），未加入分析脈絡、未解釋風險成因、行動建議僅一句話無具體步驟，則此項不得超過 18 分**；僅命中客戶但說明空洞者不得超過 20 分
                - **Token 效率／資訊密度**（15 分）：以較少 completion token 完成具體且完整的說明者得高分（資訊密度高）；**若輸出本質是資料堆砌而非分析，不論 token 多寡，此項不得超過 6 分**；token 少但內容豐富勝過 token 多但空泛
