...
public class InsightService {
    ...
    public SseEmitter streamModelScore(Dtos.AiScoreRequest request) {
        ...
        sb.append("""
                ...
                ## 評分標準（滿分 100）
                - **回應速度**（20 分）：首字延遲越低、總耗時越短越佳
                - **資料準確性**（25 分）：是否正確識別高風險客戶（嚴格對照上方原始資料），風險數字有無捏造或遺漏
                - **內容品質與深度**（40 分）：評分前請先判斷輸出類型：
                  - 「資料堆砌型」定義：行動建議僅為一句口號（如「今日排續約會」「催回覆時程」「安排高層介入」），未說明為何此時此刻需要此行動、未解釋風險數字背後的業務含義、未提供具體步驟或時間節點。此類型**不論客戶是否命中，內容品質最高 15 分**。
                  - 「分析報告型」定義：說明篩選邏輯、解釋風險成因（如「已逾五個月未接觸」「續約日已逾期 X 個月」）、建議行動有具體對象/方式/時限。此類型正常評分，優秀者可達 35–40 分。
                  - 中間型（有部分分析但不完整）：15–28 分區間視深度給分。
                - **Token 效率／資訊密度**（15 分）：以較少 completion token 完成具體且完整說明者得高分；**「資料堆砌型」輸出不論 token 多寡，此項最高 5 分**；使用較少 token 但達到「分析報告型」標準者，此項可得滿分
        ...
        sb.append("""
                ...
                1. **評分總表**（每個模型四個維度分數 + 總分，表格格式；欄位：回應速度/20、資料準確性/25、內容品質與深度/40、Token效率/15、總分/100）
                2. **逐模型評語**（速度表現、資料命中情況、內容深度與可行性亮點、不足之處）
                3. **最終排名與綜合建議**（哪個模型最適合此類 CRM 報告任務，及理由）
                使用繁體中文，語氣客觀，不偏袒特定廠商。
                """);
